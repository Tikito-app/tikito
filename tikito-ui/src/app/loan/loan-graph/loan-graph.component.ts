    ...
    return Object.keys(seriesValuesByName)
      .map((name: any) => {
        let group: any = {
          ...
          areaStyle: {
            color: name.endsWith('interest') || name.endsWith('interest-simulated') ? Util.COLORS[1] : Util.COLORS[0],
            opacity: name.endsWith('simulated') ? 0.3 : 1
          }
        };
        return group;
      })
    ...
    return {
      ...
      formatter(params: any): any {
        ...
        if (param.value != 0) {
          ...
          values.forEach((value: LoanValue) => {
            let loanPart = loanPartsById[value.loanPartId];

            html += `${loanPart.name} <br/>`;
            html += `Interest paid <span style="float: right; margin-left: 20px; color: ${Util.currencyColor(value.interestPaid)};">${Util.currencyFormat(value.interestPaid)}</span><br/>`;
            html += `Interest this period <span style="float: right; margin-left: 20px; color: ${Util.currencyColor(value.interestPaidThisPeriod)};">${Util.currencyFormat(value.interestPaidThisPeriod)}</span><br/>`;
            html += `Interest remaining <span style="float: right; margin-left: 20px; color: ${Util.currencyColor(value.interestRemaining)};">${Util.currencyFormat(value.interestRemaining)}</span><br/>`;
            html += `Payment remaining <span style="float: right; margin-left: 20px; color: ${Util.currencyColor(value.amountRemaining)};">${Util.currencyFormat(value.amountRemaining)}</span><br/>`;
            html += `Payment done <span style="float: right; margin-left: 20px; color: ${Util.currencyColor(value.loanPaid)};">${Util.currencyFormat(value.loanPaid)}</span><br/>`;
            html += `Payment this period <span style="float: right; margin-left: 20px; color: ${Util.currencyColor(value.loanPaidThisPeriod)};">${Util.currencyFormat(value.loanPaidThisPeriod)}</span><br/>`;
            totalRepaid += value.loanPaid;
            totalInterestPaid += value.interestPaid;
          });
          html += '<hr>';
          html += `Repaid <span style="float: right; margin-left: 20px; color: ${Util.currencyColor(totalRepaid)};">${Util.currencyFormat(totalRepaid)}</span><br/>`;
